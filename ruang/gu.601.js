...
router.get('/', (req, res) => {
  const kodeRuangan = 'GU.601'; // Ganti sesuai kebutuhan

  // Query untuk ambil kegiatan yang paling dekat dengan waktu sekarang
  const ongoingQuery = `
    SELECT *, ABS(TIMESTAMPDIFF(SECOND, start_time, CURTIME())) AS selisih_detik,
      CASE
    ...
    WHERE tanggal_pinjam = CURDATE()
      AND kode_ruangan = ?
    ORDER BY selisih_detik ASC
    LIMIT 1
  `;

  // Query untuk ambil kegiatan berikutnya setelah waktu sekarang
  const nextQuery = `
    SELECT *, ABS(TIMESTAMPDIFF(SECOND, start_time, CURTIME())) AS selisih_detik,
      CASE
        WHEN jenis_kegiatan = 'lainnya' THEN nama_kegiatan_other
  ...
  connection.query(ongoingQuery, [kodeRuangan], (err, ongoingResult) => {
    if (err) {
      return res.send('Error retrieving ongoing data!');
    }

    connection.query(nextQuery, [kodeRuangan], (err, nextResult) => {
      if (err) {
